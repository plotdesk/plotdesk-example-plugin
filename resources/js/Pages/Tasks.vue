...
<template>
    <div class="tasks">
        <header class="tasks__header">
            <div>
                <h1 class="tasks__title">Example Tasks</h1>
                <p class="tasks__subtitle">Manage tasks managed by the plotdesk example plugin.</p>
            </div>
        ...
        <form v-if="showForm" class="pd-card pd-form" @submit.prevent="submit">
            <div class="pd-form__row">
                <label class="pd-form__label">
                    <span>Title</span>
                    <input v-model="form.title" type="text" required class="pd-input" placeholder="What needs to be done?" />
                </label>
            </div>
            <div class="pd-form__row">
                <label class="pd-form__label">
                    <span>Description</span>
                    <textarea v-model="form.description" class="pd-input" rows="2" placeholder="Optional details..." />
                </label>
            </div>
        ...
        <section class="pd-card">
            <div v-if="tasks.length === 0" class="pd-empty">No tasks yet. Create one to get started.</div>
            <table v-else class="pd-table">
                ...
                <tbody>
                    <tr v-for="task in tasks" :key="task.id">
                        ...
                        <td><span class="pd-chip" :class="priorityStyle(task.priority)">{{ task.priority }}</span></td>
                        <td><span class="pd-chip" :class="statusStyle(task.status)">{{ task.status }}</span></td>
                        <td>{{ task.due_date || '—' }}</td>
                        <td>
                            <button type="button" class="pd-btn pd-btn--ghost pd-btn--danger" @click="removeTask(task)">Delete</button>
                        </td>
                    </tr>
                </tbody>
            </table>
...
.pd-btn--primary:hover:not(:disabled) { background: #081f66; }
